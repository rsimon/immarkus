...
import { getLabel, getPropertyValue } from './utils/resourceUtils';
import { getImages } from './utils/canvasUtils';
import { parseImageService } from './utils';
...
export const parseURL = async (input: string): Promise<CozyParseResult> => {
  try {
    new URL(input);
  } catch {
    return {
...
const parseImageResource = (resource: any) => {
  ...
  if (service) {
    return {
      ...
      serviceUrl: getPropertyValue<string>(resource, 'id')
